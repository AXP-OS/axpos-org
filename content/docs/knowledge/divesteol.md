...
- [ ] Hypatia (not directly AXP.OS related) [3]
    ...
    - [ ] automating signature db updates
...
and it already has been [started to work](https://github.com/sfX-android/automation_scripts/tree/ansible/roles/kernel_patcher) on that topic. From what is known currently it *should* be possible to achieve continuing this part and even automate all or whole parts at least. As the process has just been started with it is unclear what challenges are behind the next corner though. **Even though the process of patching itself IS release critical the January build will not contain any new Kernel patches to not delay releases even more.**
