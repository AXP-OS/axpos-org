...
1. reboot to the bootloader (**not** fastboot**d**), note down the current _BOOT SLOT_ name (`a|b`)
1. flash AXP.OS - this also includes the AXP.OS signing key so you can lock your device - **if supported**
   - when on Windows, type: `flash-all.bat`
   - when on Linux, type: `./flash-all.sh`
1. during the process you will see the _SLOT_ which gets flashed, here an excerpt where it flashes slot **>b<** (e.g. boot_**b**):
```
